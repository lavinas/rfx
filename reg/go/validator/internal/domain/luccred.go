...
type LucrCred struct {
	...
	DiscountRevenue    float64 `gorm:"column:gross_revenue;type:numeric(18,2)"`
	DiscountRevenueInt int64   `fixed:"6,17"`
	RentRevenue        float64 `gorm:"column:rental_revenue;type:numeric(18,2)"`
	RentRevenueInt     int64   `fixed:"18,29"`
	OtherRevenue       float64 `gorm:"column:others_revenue"`
	...
	OtherCost          float64 `gorm:"column:others_cost"`
	OtherCostInt       int64   `fixed:"102,113"`
}

// TableName returns the name of the database table for the LucrCred model.
...
func (l *LucrCred) GetDB(repo port.Repository, year int, quarter int) (map[string]port.Report, error) {
