...
create table cadoc_6334_v2.tmp_intercam as
select extract(year from a.transaction_date) as year,
       extract(quarter from a.transaction_date) as quarter,
       coalesce (c.produto_final, 31) as product_code,
       coalesce (c.modalidade_final, 'P') as card_type,
       ...
       round(avg(a.cost_interchange_value / a.transaction_amount * 100), 2) as interchange_fee,
...
select count(1) as total_records
  from cadoc_6334_v2.tmp_intercam;
...
left join cadoc_6334_v2.tmp_intercam b
  on a.year = b.year
...
select count(1)
  from cadoc_6334_v2.tmp_intercam a
left join cadoc_6334_v2.intercam b
  on a.year = b.year
  and a.quarter = b.quarter
  and a.product_code = b.product_code
  and a.card_type = b.card_type
...
select min(a.interchange_fee - b.interchange_fee) as min_diff_interchange_fee,
       max(a.interchange_fee - b.interchange_fee) as max_diff_interchange_fee,
  ...
  from cadoc_6334_v2.intercam a
inner join cadoc_6334_v2.tmp_intercam b
  on a.year = b.year
  and a.quarter = b.quarter
  and a.product_code = b.product_code
  and a.card_type = b.card_type
...
drop table cadoc_6334_v2.tmp_intercam;