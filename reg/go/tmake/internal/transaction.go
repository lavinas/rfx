...
// GetInsertRow returns the SQL insert statement for the transaction
func (t *Transaction) GetInsertRow() string {
	str := "("
	str += fmt.Sprintf("%d, ", t.ID)
	str += fmt.Sprintf("'%s', ", t.CreatedAt.Format("2006-01-02 15:04:05"))
	str += fmt.Sprintf("'%s', ", t.UpdatedAt.Format("2006-01-02 15:04:05"))
	str += fmt.Sprintf("'%s', ", t.Key1)
	str += fmt.Sprintf("%d, ", t.EstablishmentCode)
	str += fmt.Sprintf("%d, ", t.EstablishmentNature)
	str += fmt.Sprintf("%d, ", t.EstablishmentMCC)
	str += fmt.Sprintf("%d, ", t.EstablishmentTerminalCode)
	str += fmt.Sprintf("%d, ", t.BIN)
	str += fmt.Sprintf("'%s', ", t.AuthorizationCode)
	str += fmt.Sprintf("'%s', ", t.TransactionNSU)
	str += fmt.Sprintf("'%s', ", t.TransactionDate.Format("2006-01-02 15:04:05"))
	str += fmt.Sprintf("%.2f, ", t.TransactionAmount)
	str += fmt.Sprintf("%d, ", t.TransactionInstallments)
	str += fmt.Sprintf("'%s', ", t.TransactionBrand)
	str += fmt.Sprintf("'%s', ", t.TransactionProduct)
	str += fmt.Sprintf("'%s', ", t.TransactionCapture)
	str += fmt.Sprintf("%.2f, ", t.RevenueMDRValue)
	str += fmt.Sprintf("%.2f, ", t.CostInterchangeValue)
	str += fmt.Sprintf("%d, ", t.HighSourcePriority)
	str += fmt.Sprintf("%d, ", t.StatusID)
	str += fmt.Sprintf("'%s', ", t.StatusName)
	str += fmt.Sprintf("%d, ", t.StatusCount)
	if t.PeriodDate != nil {
		str += fmt.Sprintf("'%s', ", t.PeriodDate.Format("2006-01-02 15:04:05"))
	} else {
		str += "NULL, "
	}
	if t.PeriodClosingID != nil {
		str += fmt.Sprintf("%d, ", *t.PeriodClosingID)
	} else {
		str += "NULL, "
	}
	str += fmt.Sprintf("'%s'", t.TransacID)
	str += ")"
	return str
